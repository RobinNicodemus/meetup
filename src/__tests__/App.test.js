...
describe('<App /> integration', () => {
  ...
  test('get list of events after user chooses number of events', () => {
    const AppWrapper = mount(<App />);
    AppWrapper.instance().updateEvents = jest.fn();
    AppWrapper.instance().forceUpdate();
    const NumberOfEventsWrapper = AppWrapper.find(NumberOfEvents);
    NumberOfEventsWrapper.instance().handleInputChanged({ target: { value: 8 } });
    //timeout is necessary because:
    //updateEvents is a "debounced" func in NumberOfEvents.js
    setTimeout(() => {
      expect(AppWrapper.instance().updateEvents).toHaveBeenCalledTimes(1);
      expect(AppWrapper.instance().updateEvents).toHaveBeenCalledWith(null, null, 8);
    }, 500);
  });
